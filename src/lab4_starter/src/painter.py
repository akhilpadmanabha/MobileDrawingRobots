...
def callback_robot0_status(message):
    global r0status
    r0status = message.data

def get_robot0status():
    rospy.Subscriber("/robot0/status", String, callback_robot0_status) 

def callback_robot0_position(message):
    global r0position
    r0x = message.position[0]
    r0y = message.position[1]
    r0position = [r0x, r0y]

def get_robot0position():
    rospy.Subscriber("/robot0/position", msg_type_position, callback_robot0_position) 
...
class OccupancyGrid2d(object):
    ...
    def LoadParameters(self):
        ...
        if not rospy.has_param("~y/num"):
            return False
        self._y_num = rospy.get_param("~x/num")
    ...
    def SensorCallback(self, msg):
        ...
        for idx, r in enumerate(msg.ranges):
            ...
            # TODO
            #angle = msg.angle_min + msg.angle_increment * idx
            #sangle = angle + yaw
            ...
            val = self.PointToVoxel(r0position)
            self._map[val] = self._occupied_threshold
            #val0 = self.PointToVoxel(sensor_x+1, sensor_y+1)
		    #self._map[val0] = self._free_threshold
            '''
            x_val = 0;
            y_val = 0;

            for r_i in np.arange(r, 0, -min(self._x_res, self._y_res)):
                x_last = x_val
                y_last = y_val
                x_val = r_i*np.sin(angle) + sensor_x
                y_val = r_i*np.cos(angle) + sensor_y

                val = self.PointToVoxel(x_val, y_val)
                if self.PointToVoxel(x_val, y_val) != self.PointToVoxel(x_last, y_last):
                    if r_i == r:
                        self._map[val] = self._map[val] + self._occupied_update
                        if self._map[val] >= self._occupied_threshold:
                            self._map[val] = self._occupied_threshold
                    else:
                        self._map[val] = self._map[val] + self._free_update
...
if __name__ == '__main__':
    ...
    #robot0trajectories = comp_vision()
    robot0trajectories = [[[5,3], [4,2]],[[1,3], [2,4]], [[4,4], [3,5]], [[3,1], [2,2]]]  
    #robot0trajectories = [[[1,5]], [[2,3], [3,5], [4,2]], [[1, 2], [3, 3]]]#this is the trajectory produced by the CV
    #robot0trajectories = [[[1, 1]], [[3,5], [5,1], [1,3], [5,3], [1,1]]]
    while not rospy.is_shutdown():
        get_robot0status()
        get_robot0position() #this listens for robot0 current positiion
        send_robot0trajectory() #this publishes a trajectory for robot0
        r = rospy.Rate(10)
        r.sleep()